// Generated by Haxe 4.2.1+bf9ff69
#ifndef INCLUDED_CombatHUD
#define INCLUDED_CombatHUD

#ifndef HXCPP_H
#include <hxcpp.h>
#endif

#ifndef INCLUDED_flixel_group_FlxTypedGroup
#include <flixel/group/FlxTypedGroup.h>
#endif
HX_DECLARE_CLASS0(CombatHUD)
HX_DECLARE_CLASS0(Enemy)
HX_DECLARE_CLASS0(Outcome)
HX_DECLARE_CLASS0(Pointer)
HX_DECLARE_CLASS0(Text)
HX_DECLARE_CLASS1(flixel,FlxBasic)
HX_DECLARE_CLASS1(flixel,FlxObject)
HX_DECLARE_CLASS1(flixel,FlxSprite)
HX_DECLARE_CLASS3(flixel,addons,text,FlxTypeText)
HX_DECLARE_CLASS2(flixel,group,FlxTypedGroup)
HX_DECLARE_CLASS2(flixel,_hx_system,FlxSound)
HX_DECLARE_CLASS2(flixel,text,FlxText)
HX_DECLARE_CLASS2(flixel,tweens,FlxTween)
HX_DECLARE_CLASS2(flixel,util,IFlxDestroyable)
HX_DECLARE_CLASS1(haxe,IMap)
HX_DECLARE_CLASS2(haxe,ds,BalancedTree)
HX_DECLARE_CLASS2(haxe,ds,EnumValueMap)



class HXCPP_CLASS_ATTRIBUTES CombatHUD_obj : public  ::flixel::group::FlxTypedGroup_obj
{
	public:
		typedef  ::flixel::group::FlxTypedGroup_obj super;
		typedef CombatHUD_obj OBJ_;
		CombatHUD_obj();

	public:
		enum { _hx_ClassId = 0x7e771c1f };

		void __construct();
		inline void *operator new(size_t inSize, bool inContainer=true,const char *inName="CombatHUD")
			{ return ::hx::Object::operator new(inSize,inContainer,inName); }
		inline void *operator new(size_t inSize, int extra)
			{ return ::hx::Object::operator new(inSize+extra,true,"CombatHUD"); }
		static ::hx::ObjectPtr< CombatHUD_obj > __new();
		static ::hx::ObjectPtr< CombatHUD_obj > __alloc(::hx::Ctx *_hx_ctx);
		static void * _hx_vtable;
		static Dynamic __CreateEmpty();
		static Dynamic __Create(::hx::DynamicArray inArgs);
		//~CombatHUD_obj();

		HX_DO_RTTI_ALL;
		::hx::Val __Field(const ::String &inString, ::hx::PropertyAccess inCallProp);
		::hx::Val __SetField(const ::String &inString,const ::hx::Val &inValue, ::hx::PropertyAccess inCallProp);
		void __GetFields(Array< ::String> &outFields);
		static void __register();
		void __Mark(HX_MARK_PARAMS);
		void __Visit(HX_VISIT_PARAMS);
		bool _hx_isInstanceOf(int inClassId);
		::String __ToString() const { return HX_("CombatHUD",c3,d4,6c,0c); }

		 ::Enemy enemy;
		 ::flixel::FlxSprite background;
		 ::Enemy enemySprite;
		 ::flixel::_hx_system::FlxSound check;
		 ::flixel::_hx_system::FlxSound next;
		 ::Pointer pointer;
		 ::haxe::ds::EnumValueMap choices;
		::Array< ::String > ynCho;
		 ::flixel::FlxSprite pointerRight;
		 ::flixel::FlxSprite pointerLeft;
		 ::Outcome outcome;
		int state;
		 ::Text combatText;
		::Array< ::String > dilog_boxes;
		::String name;
		bool txt;
		bool textIn;
		 ::flixel::FlxSprite enterCur;
		 ::flixel::FlxSprite nftStyle;
		int nftStyleNum;
		bool rodTalk;
		int rodNum;
		 ::flixel::text::FlxText rodNumText;
		Float bananaCoin;
		Float appleCoin;
		Float dexCoin;
		int starterPrize;
		int dexStarterPirze;
		bool starterTalk;
		bool buyStarter;
		bool starterInStreet;
		int investNum;
		 ::flixel::text::FlxText investNumText;
		 ::flixel::text::FlxText enemyNameText;
		 ::flixel::text::FlxText diamondText;
		 ::flixel::text::FlxText diamondUiText;
		Float diamond;
		Float alpha;
		bool wait;
		void initCombat(Float diamond, ::flixel::text::FlxText diamondUiText,Float bananaCoin,Float appleCoin,Float dexCoin, ::Enemy enemy);
		::Dynamic initCombat_dyn();

		void finishFadeIn( ::flixel::tweens::FlxTween _);
		::Dynamic finishFadeIn_dyn();

		void updateKeyboardInput();
		::Dynamic updateKeyboardInput_dyn();

		void lrPointer(Float spriteX,Float spriteY,Float spriteW,Float spriteH);
		::Dynamic lrPointer_dyn();

		void updateAlpha(Float alpha);
		::Dynamic updateAlpha_dyn();

		void finishFadeOut( ::flixel::tweens::FlxTween _);
		::Dynamic finishFadeOut_dyn();

		void update(Float elapsed);

		void doneResultsIn();
		::Dynamic doneResultsIn_dyn();

};


#endif /* INCLUDED_CombatHUD */ 
