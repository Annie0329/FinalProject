...
class CombatHUD extends FlxTypedGroup<FlxSprite>
{
	...
	var investText:FlxText;

	public var investNum:Int = 0;
	...
	public function new()
	{
		...
		investText = new FlxText(background.x + 60, background.y + 96, 170, "0", 44);
		investText.color = 0xff2D5925;
		add(investText);
		investText.visible = false;
	...
	function updateKeyboardInput()
	{
		...
		else if ((left || right) && investText.visible)
		{
			if (left && investNum != 0)
				investNum -= 50;
			else if (right)
				investNum += 50;
			investText.text = Std.string(investNum);
		}
	}
