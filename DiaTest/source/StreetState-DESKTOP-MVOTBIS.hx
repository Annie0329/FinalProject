...
class StreetState extends FlxState
{
	...
	override public function update(elapsed:Float)
	{
		...
		// 除錯大隊
		ufo.text = Std.string(bag.bananaCoin);
	...
	function updateTalking()
	{
		...
		if (talkYes && enter && !bag.bagUi.visible && !bag.shopUi.visible)
		{
			...
			if (npcType == p1Mach)
			{
				dia.getDiamond(bag.diamondCounter, bag.diamondText, bag.bananaCoin);
				ufo.visible = true;
				ufo.text = Std.string(dia.diamond);
			}
			dia.context(npcType);
		}
	}
