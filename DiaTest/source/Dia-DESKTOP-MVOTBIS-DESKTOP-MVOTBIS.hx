...
class Dia extends FlxTypedGroup<FlxSprite>
{
	...
	// 香蕉換能量
	var bck = 100000;
	var bcBananaCoinIn:Int = 1000; // 機器裡有多少香蕉
	var bcCoinIn:Float = 10;

	// 蘋果換能量
	var ack = 8000;
	var acAppleCoinIn:Int = 2; // 機器裡有多少APS，蘋果就是APS
	var acCoinIn:Float = 4000;

	// 能量換蘋果
	var cak = 8000;
	var caCoinIn:Int = 2;
	var caAppleCoinIn:Float = 4000;

	var coinOut:Int = 1; // 玩家決定要塞多少幣進去
	var p1Gain:Float = 0; // 玩家到底從小屋1號機器得到多少錢
	...
	public function context(npcType:NPC.NpcType)
	{
		...
		switch (npcType)
		{
			...
			case mathChart:
				...
				txt = false;
			case p1BaToCoMach:
				// p1Prize = FlxMath.roundDecimal(bck / bcBananaCoinIn, 2);
				if (bananaCoin > 0)
				{
				...
				txt = false;
			case p1ApToCoMach:
				// p1Prize = FlxMath.roundDecimal(ack / acAppleCoinIn, 2);
				if (appleCoin > 0)
				{
					name = ':N:你想用多少APS幣買能量幣？你目前有 $appleCoin APS幣'; // ， 1 APS幣可買 $p1Prize 能量幣。';
					coinText.visible = true;
				}
				else
					name = ":N:你沒有APS幣。";
				txt = false;
			case p1CoToApMach:
				// p1Prize = FlxMath.roundDecimal(cak / caCoinIn, 2);
				if (diamond > 0)
				{
					name = ':N:你想用多少能量幣買APS幣？你目前有 $diamond 能量幣'; // ， 1 能量幣可買 $p1Prize APS幣。';
					coinText.visible = true;
				}
				else
					name = ":N:你沒有能量幣。";
	...
	function updateEnter()
	{
		...
		if (enter && textRunDone)
		{
			...
			else if (coinText.visible)
			{
				...
				{
					p1Gain = FlxMath.roundDecimal(bcCoinIn - (bck / (bcBananaCoinIn + coinOut)), 2);
					name = ':N:你得到了 $p1Gain 能量幣。';
					txt = false;
					show(name, txt);
				...
				{
					p1Gain = FlxMath.roundDecimal(acCoinIn - (ack / (acAppleCoinIn + coinOut)), 2);
					name = ':N:你得到了 $p1Gain 能量幣。';
					txt = false;
					show(name, txt);
				...
				{
					p1Gain = FlxMath.roundDecimal(caAppleCoinIn - (cak / (caCoinIn + coinOut)), 2);
					name = ':N:你得到了 $p1Gain APS幣。';
					txt = false;
					show(name, txt);
